...
np.savetxt(fold+'/'+tet_name+'.res.'+tet,res,fmt='%i')
np.savetxt(fold+'/'+tet_name+'.clu.'+tet,[max(clu)+1]+list(clu+1),fmt='%i')
# TODO: HEADER DEPENDING ON NUMBER OF FEATURES !
np.savetxt(fold+'/'+tet_name+'.fet.'+tet,d,fmt='%i',header = str(nchan*npc+4+1),comments='')
np.savetxt(fold+'/'+tet_name+'.mm.'+tet,[0,0,0,0,0,0,0,0])
...
with open(fold+'/'+tet_name+'.spk.'+tet, "wb") as binary_file:
...
#with open(fold+'/'+tet_name+'.spkb.'+tet, "wb") as binary_file:
    # Write text or bytes to the file
